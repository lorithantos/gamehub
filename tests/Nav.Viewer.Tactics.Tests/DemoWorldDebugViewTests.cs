...
public sealed class DemoWorldDebugViewTests
{
    ...
    private static string Dump(IReadOnlyList<DebugRow> rows) =>
        string.Join("\n", rows.Select(r => $"{r.Group,-11} {r.Key,-14} {r.Value}"));
    ...
    public void AUnitWithAKitATargetAndASideUnderFogSaysAllOfIt()
    {
        ...
        var health = Row(rows, "Unit", "health");
        Assert.Contains("%", health, StringComparison.Ordinal);
        ...
        var weapon = Row(rows, "Kit", "weapon");
        Assert.Contains("rifle", weapon, StringComparison.Ordinal);
        ...
        Assert.Contains("steps it can shoot", Row(rows, "Kit", "range"), StringComparison.Ordinal);
        ...
        var sight = Row(rows, "Kit", "sight");
        Assert.Contains("6", sight, StringComparison.Ordinal);
        Assert.Contains("2 past its own reach", sight, StringComparison.Ordinal);

        Assert.Contains("2 shots a second", Row(rows, "Kit", "rate of fire"), StringComparison.Ordinal);
        ...
        var target = Row(rows, "Fight", "target");
        Assert.Contains("unit 1", target, StringComparison.Ordinal);
        Assert.Contains("side 1", target, StringComparison.Ordinal);
        ...
        var contribution = Row(rows, "Unit", "contribution");
        Assert.Contains("points banked", contribution, StringComparison.Ordinal);
        ...
        var exposed = Row(rows, "Unit", "exposed");
        Assert.Contains("4 ticks", exposed, StringComparison.Ordinal);
        Assert.Contains("scripted threat", exposed, StringComparison.Ordinal);
        ...
        Assert.Contains("2 cells hostile to side 0", Row(rows, "Perception", "can see"), StringComparison.Ordinal);
        Assert.Contains("1 enemy units", Row(rows, "Perception", "remembers"), StringComparison.Ordinal);
        Assert.Contains("1 it can plan to reach", Row(rows, "Perception", "pads in view"), StringComparison.Ordinal);
    ...
    public void AnUnknownAgentIsAnsweredRatherThanRefused()
    {
        ...
        Assert.Contains("-4", only.Value, StringComparison.Ordinal);
        Assert.Contains("no such unit", only.Value, StringComparison.Ordinal);
    ...
    public void TheWorldRowsCarryTheRatesAndTheRankTableAUnitIsMeasuredAgainst()
    {
        ...
        Assert.Contains("1 scripted cells", Row(rows, "World", "threats"), StringComparison.Ordinal);
        Assert.Contains("1 repair cells", Row(rows, "World", "pads"), StringComparison.Ordinal);

        // The rates a tick applies, each with the tick in it, because a rate
        // without its period is not a rate.
        Assert.Contains("a tick", Row(rows, "Rates", "repair"), StringComparison.Ordinal);
        Assert.Contains("a tick", Row(rows, "Rates", "damage"), StringComparison.Ordinal);
        Assert.Contains("a tick", Row(rows, "Rates", "self-heal"), StringComparison.Ordinal);
